...
public class jTPCC implements jTPCCConfig
{
	...
	if(Integer.parseInt(limPerMin) !=0){
	    ...
	    log.info("limPerMin_Terminal is calculated= " +  limPerMin);
	}
	else{
	    limPerMin_Terminal = -1;
	    log.info("#####-1, limPerMin_Terminal= " +  limPerMin);
	}
	...
	if(databaseDriverLoaded)
	{
		...
		if(Long.parseLong(iRunMins) != 0 && Integer.parseInt(iRunTxnsPerTerminal) == 0)
		{
			...
			executionTimeMillis = Long.parseLong(iRunMins) * 60000;
	                log.info("!!!!!!!!!!!!#####-1, executionTimeMillis= " +  executionTimeMillis + "    ");
		...
		try
		{
		    ...
		    sessionEndTargetTime = executionTimeMillis;
	            log.info("!!!!!!!!!!!!#####2, executionTimeMillis= " +  sessionEndTargetTime + "    ");
			...
			sessionEndTargetTime += sessionStartTimestamp;
	            ...
	            log.info("!!!!!!!!!!!!#####3, executionTimeMillis= " +  sessionEndTargetTime + "    ");
	...
	synchronized(terminals)
	{
	    log.info("!!!!!!!!!!!!##### call signalTerminalsRequestEnd");
	    if(!signalTerminalsRequestEndSent)
	    {
	    log.info("!!!!!!!!!!!!##### call signalTerminalsRequestEnd 2");
		if(timeTriggered)
...
long cur = System.currentTimeMillis();
	if(sessionEndTargetTime != -1 && cur > sessionEndTargetTime)
	{
	    log.info("!!!!!!!!!!!!##### end, current time = " +  cur  + "    ");
